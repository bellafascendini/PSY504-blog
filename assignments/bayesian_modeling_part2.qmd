...
output-file: bayesian_modeling_part2.html
format:
  html:
    code-fold: true
    code-overflow: wrap
...
1. Likelihood distribution: $P(D|\theta)$, which is the probability of observing data $D$ given parameters $\theta$.
2. Priors: $P_0(\theta)$, which is the prior belief about parameters before seeing what the data looks like.
3. Prior predictive distribution: $P_0(D) = \int P(D|\theta)P_0(\theta)d\theta$, which is the expected distribution of the data given only prior beliefs.
4. Posteriors: $P(\theta|D) = \frac{P(D|\theta)P_0(\theta)}{P(D)}$, which is the updated beliefs about parameters after seeing data.
5. Posterior predictive distribution: $P(D_{\text{new}}|D) = \int P(D_{\text{new}}|\theta)P(\theta|D)d\theta$, which is the expected distribution of new data given the posteriors.
...
draws_gentoo <- as_draws_df(fit_gentoo) 
...
draws_gentoo %>% 
  ...
  xlab("parameter space") +
  facet_wrap(~ parameter, scales = "free", labeller = label_parsed) +
  labs(title = "Posterior Distributions for Gentoo Penguins Model")
```
```{r}
#2. Conditional effects plot
...
fitted_gentoo <- fitted(fit_gentoo, newdata = nd_gentoo) %>% 
  data.frame() %>% 
  bind_cols(nd_gentoo) 
...
fitted_gentoo %>%
  ...
  geom_point(data = gentoo,
             aes(y = bill_length_mm)) +
  labs(title = "Fitted Values for Gentoo Penguins",
       x = "Body Mass (g)",
       y = "Bill Length (mm)")
```
```{r}
#3. Posterior predictive distribution plot

predict_gentoo <- predict(fit_gentoo, newdata = nd_gentoo) %>% 
  data.frame() %>% 
  bind_cols(nd_gentoo)
...
predict_gentoo %>%
  ...
  geom_ribbon(aes(ymin = Q2.5, ymax = Q97.5),
              alpha = 1/4) +
  geom_ribbon(data = fitted_gentoo,
              aes(ymin = Q2.5, ymax = Q97.5),
              alpha = 1/4, fill = "blue") +
  geom_line(data = fitted_gentoo,
            aes(y = Estimate)) +
  geom_point(data = gentoo,
             aes(y = bill_length_mm)) +
  coord_cartesian(ylim = range(gentoo$bill_length_mm)) +
  labs(title = "Posterior Predictive Distribution for Gentoo Penguins",
       subtitle = "Light blue band: 95% interval for conditional mean\nLight gray band: 95% interval for predictive distribution",
       x = "Body Mass (g)",
       y = "Bill Length (mm)")
```
