...
output-file: "multilevel_modeling_walkthrough.html"
...
```{r}
#ENTER YOUR ATTEMPT AND THOUGHTS HERE: The red lines are identical across all toy types, which suggests that this model ignores toy-type differences. It assumes a single relationship between hours of practice and reading age for all toy types, which does not reflect the data accurately. Observing the plots, it is clear that different toy types have varying intercepts and possibly different slopes as well. This suggests we need models that account for group-level variations such as fixed effects or mixed effects models.
...
#Fixed effects: Each toy type has a unique intercept but shares a common slope. This leads to vertically shifted lines across toy types, which accounts for differences in reading age but not how practice might affect each type differently.
#Random intercepts: The variance is explicitly modeled around a common slope. Each toy type has its own baseline (intercept), modeled as random deviations from an average intercept, which captures the group-level variability.
#Random slopes: This model allows both intercepts and slopes to vary for each toy type, which captures the potential differences in how 'practice' impacts 'reading age' differently across toy types, providing a more accurate representation of the data.
...
#A1: SD of random intercepts from all toy-type lines to the black line - this is the variation in intercepts across toy types.
#A2: Residual SD from all the individual datapoints (toys) to their respective toy-type lines  
#A3: Intercept - where the black line cuts the y axis  
#A4: Slope of the black line – represents the average effect of practice hours on reading age, assumed constant across all toy types.
...
num_participants <- n_distinct(efdat$PID)
```
...
```{r}
# Average and minimum number of trials per participant:
trials_summary <- efdat %>%
  count(PID) %>%
...
```{r}
audio_per_participant <- efdat %>%
  count(PID, audio) %>%
  group_by(PID) %>%
  summarise(has_all_audio = all(c("no_audio", "white_noise", "music") %in% audio)) %>%
  summarise(all_have_audio_types = all(has_all_audio))
...
list(
  ...
  all_audio_types_present = audio_per_participant$all_have_audio_types
)
```
...
The outcome of interest is the executive functioning measure, specifically the SDMT scores.

- What variables are we seeking to investigate in terms of their impact on the outcome?    
The variables we're interested in are the type of audio (no audio, white noise, music)and whether noise-cancelling headphones are used (headphones vs speakers).

- What are the units of observations?  
Each individual trial completed by a participant is an observation.

- Are the observations clustered/grouped? In what way?  
Yes, observations are clustered by participant.

- What varies *within* these clusters?  
Type of audio.

- What varies *between* these clusters?  
Participants either always use headphones or speakers.
...
```{r}
sdmt_mod <- lmer(SDMT ~ audio * headphones + (1 + audio | PID), data = efdat)
summary(sdmt_mod)
```
...
```{r}
library(lmerTest)
sdmt_mod_pvals <- lmer(SDMT ~ audio * headphones + (1 + audio | PID), data = efdat)
summary(sdmt_mod_pvals)
```
...
audio_effects <- effect("audio*headphones", sdmt_mod_pvals)
audio_effects_df <- as.data.frame(audio_effects)

ggplot(audio_effects_df, aes(x = audio, y = fit, color = headphones, group = headphones)) +
  geom_point(position = position_dodge(width = 0.2), size = 3) +
  geom_errorbar(aes(ymin = lower, ymax = upper), position = position_dodge(width = 0.2), width = 0.1) +
  labs(x = "Audio Condition", y = "Predicted SDMT Score", color = "Listening Device") +
  theme_minimal()
```
...
**Brief decription of the study**
The data analyzed in this study came from multiple healthy adult volunteers who each completed several trials of the Symbol Digit Modalities Test (SDMT). During each trial, participants were randomly assigned to perform the test while exposed to one of three audio conditions: no audio, white noise, or music. Additionally, participants were allocated into two distinct groups: one group consistently used noise-cancelling headphones, while the other group consistently used standard speakers. Each participant provided between 10 and 14 trials, with an average of approximately 12 trials per participant. Importantly, all participants had some exposure to each audio condition.

**Research question and approach**
To address the research question - How do different types of audio interfere with executive functioning, and does this interference differ depending upon whether noise-cancelling headphones are used? - we ran a linear mixed-effects model. We chose this approach because it accounted for the repeated-measures design, where multiple observations (trials) were nested within individual participants.

**The model included:**

Fixed Effects:

- Audio condition (no audio as baseline comparison, with white noise and music as comparison conditions).
- Listening device (speakers as baseline, compared against noise-cancelling headphones).
- Interaction between audio condition and listening device to explicitly test whether the effect of audio conditions on executive functioning differs depending upon headphone use.

Random Effects:

- Participant-level random intercepts, allowing baseline SDMT scores to vary among individuals, thus accounting for general individual differences in executive functioning.
- Participant-level random slopes for audio conditions, enabling the model to capture individual differences in sensitivity to audio conditions (i.e., acknowledging that some individuals may be more or less impacted by music or white noise than others).

**Results**

The statistical analysis revealed several important findings:

1. The interaction term between audio type and listening device was statistically significant (p < .001), indicating that the impact of audio conditions on executive functioning indeed differed depending on whether participants used headphones or speakers. Specifically: 

- Music, compared to the no-audio condition, significantly impaired executive functioning performance (lower SDMT scores). This negative effect of music was consistent across both headphone and speaker conditions.
- White noise did not significantly affect executive functioning performance when delivered via speakers. However, when white noise was delivered via noise-cancelling headphones, it significantly improved performance relative to the no-audio condition.

Even without any audio playing, wearing noise-cancelling headphones alone significantly enhanced executive functioning performance compared to participants who listened via standard speakers.

2. There was substantial individual variability in participants' baseline SDMT performance (random intercepts), as well as considerable variability in how participants responded to audio conditions (random slopes). This indicates meaningful differences among individuals in their susceptibility to audio interference and highlights the importance of modeling random effects at the participant level.

3. Visual and Numerical Presentation of Key Results:
- Music consistently reduced performance, regardless of whether headphones or speakers were used.
- White noise was beneficial only when combined with headphones, showing significantly higher executive functioning scores compared to no audio and speaker conditions.
- Noise-cancelling headphones alone (no audio condition) improved performance compared to speakers.

In conclusion, the present analysis demonstrates that different audio conditions significantly affect executive functioning, and this effect interacts with the method of audio delivery. Specifically, music consistently impaired executive functioning, whereas white noise offered cognitive benefits—but only when presented via noise-cancelling headphones. Additionally, the use of noise-cancelling headphones alone enhanced executive functioning, suggesting potential practical applications for minimizing environmental distractions. Substantial variability among individuals emphasizes that auditory influences on cognitive performance are subject to personal differences.
